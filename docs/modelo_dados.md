...
gold_dim_agente_simp -- gold_fato_venda_simp_agente_uf_mes -- gold_mart_simp_uf_mes
gold_dim_mercado_destino -- gold_fato_venda_simp_mercado_uf_mes -----+
                                      |
                         gold_comparacao_simp_logistica_uf_mes
```
...
O `lote_id` liga a camada Bronze às tabelas analíticas. A carga Bronze não é sobrescrita em reexecuções; o notebook de transformação usa o último lote, salvo quando o widget `lote_id` é informado. O `execucao_id` separa reprocessamentos do mesmo lote. A materialização de entrega só aceita uma execução cujo último marco esteja como `CONCLUIDA`.

## Cobertura temporal configurada

| Camada analítica | Recorte configurado | Regra de leitura |
|---|---|---|
| fatos mensais de Logística 02, SIMP e preços | janeiro de 2022 a junho de 2026 | comparações que incluem 2026 usam o primeiro semestre de todos os anos |
| fatos e marts municipais anuais | 2022 a 2024 | permanecem separados do núcleo mensal; não existem registros municipais interpolados para 2025 ou 2026 |
| cadastro de revendedores | fotografia da extração | contextual, sem série histórica reconstruída |

O intervalo acima é o parâmetro do modelo e das consultas. A cobertura efetivamente carregada deve ser confirmada no lote e nas tabelas de qualidade da execução correspondente.

## Dimensões e referências

| Tabela | Chave | Conteúdo |
|---|---|---|
| `gold_dim_tempo` | `data` | ano, mês, semana, trimestre e semestre entre janeiro de 2022 e junho de 2026 |
...
| `gold_dim_agente_simp` | `agente_chave` | agente regulado publicado no SIMP e nome normalizado para diagnóstico |
| `gold_dim_mercado_destino` | `mercado_destinatario` | categoria comercial publicada no SIMP |
...
| `gold_controle_execucao` | `execucao_id + data_evento` | início e conclusão da transformação, recorte e schema usados |
...
| `gold_fato_venda_simp_agente_uf_mes` | agente × UF × produto × mês | volume do SIMP por agente regulado |
| `gold_fato_venda_simp_mercado_uf_mes` | mercado destinatário × UF × produto × mês | volume do SIMP por canal comercial |
| `gold_mart_simp_uf_mes` | UF × produto × mês | volume SIMP, concentração de agentes e preço agregado quando disponível |
| `gold_comparacao_simp_logistica_uf_mes` | UF × produto × mês | volumes paralelos, diferença e status de comparação de escopo |
| `gold_comparacao_simp_logistica_agente_uf_mes` | agente/vendedor normalizado × UF × produto × mês | cobertura do diagnóstico nominal entre as duas fontes |
...
| SIMP e preço por UF | `data_referencia + uf + produto_analitico` | associação descritiva em mart específico, com cobertura de preço |
| SIMP e Logística 02 | `data_referencia + uf + produto_analitico` | comparação de fontes, sem substituir volume de uma pela outra |
...
## Leitura própria da base SIMP

No SIMP, a participação é calculada com o volume positivo de cada `Agente Regulado` dentro de `ano + produto_analitico` para os rankings nacionais e dentro de `UF + mês + produto` para o mart. O mercado destinatário é uma dimensão independente: ele permite separar, por exemplo, postos bandeirados, bandeira branca, consumidor final e TRR quando esses valores existem na origem.

As comparações entre SIMP e Logística 02 não entram no cálculo de HHI ou de participação de nenhuma das fontes. Elas mantêm `volume_simp_litros`, `volume_logistica_litros`, diferença percentual e um status de escopo para que a divergência continue auditável.
