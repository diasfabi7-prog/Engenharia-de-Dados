...
dbutils.widgets.text("database", "workspace.anp_combustiveis_br_2022_2026", "Database")
dbutils.widgets.text("data_inicial", "2022-01-01", "Data inicial")
dbutils.widgets.text("data_final", "2026-06-30", "Data final")
dbutils.widgets.text("ultimo_ano_municipal", "2024", "Último ano municipal")
...
DATA_INICIAL = datetime.strptime(dbutils.widgets.get("data_inicial"), "%Y-%m-%d").date()
DATA_FINAL = datetime.strptime(dbutils.widgets.get("data_final"), "%Y-%m-%d").date()
ANO_INICIAL = DATA_INICIAL.year
ANO_FINAL = DATA_FINAL.year
ULTIMO_ANO_MUNICIPAL = int(dbutils.widgets.get("ultimo_ano_municipal"))
ANO_FINAL_MUNICIPAL = min(ANO_FINAL, ULTIMO_ANO_MUNICIPAL)
...
if ULTIMO_ANO_MUNICIPAL < ANO_INICIAL:
    raise ValueError("O último ano municipal não pode ser anterior à data inicial.")
...
def write_delta(name: str, frame: DataFrame) -> None:
    frame = frame.withColumn("execucao_id", F.lit(EXECUCAO_ID))
    frame.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable(table(name))


def append_delta(name: str, frame: DataFrame) -> None:
    frame.write.format("delta").mode("append").option("mergeSchema", "true").saveAsTable(table(name))
...
EXECUCAO_ID = f"{LOTE_ID}_{uuid4().hex[:8]}"
EVENTO_INICIO = datetime.now(timezone.utc)
controle_inicio = spark.createDataFrame(
    [
        (
            EXECUCAO_ID,
            LOTE_ID,
    ...
    "execucao_id string, lote_id string, schema_destino string, data_inicial string, data_final string, "
    "ultimo_ano_municipal int, status string, data_evento timestamp, mensagem string",
)
append_delta("gold_controle_execucao", controle_inicio)
...
def decimal_from_text(column):
    ...
    return (
        ...
        .when(brazilian_format, standardized.cast(DecimalType(22, 6)))
        .otherwise(raw.cast(DecimalType(22, 6)))
    )
...
def prepare_sales(frame: DataFrame, default_product: str) -> DataFrame:
    ...
    return (
        result.withColumn("municipio_norm", normalized_text(F.col("municipio")))
        .withColumn("produto_origem_normalizado", normalized_text(F.col("produto_origem")))
        .withColumn("periodo_valido", F.col("ano").isNotNull())
        .withColumn(
            "dentro_do_recorte",
            F.coalesce(F.col("ano").between(ANO_INICIAL, ANO_FINAL_MUNICIPAL), F.lit(False)),
        )
    )
...
dim_municipio = (
    venda_silver.filter(
        F.col("periodo_valido") & F.col("dentro_do_recorte") & F.col("codigo_ibge").rlike(r"^\d{7}$")
    )
...
preco_base = (
    ...
    .withColumn("periodo_valido", F.col("data_coleta").isNotNull())
    .withColumn(
        "dentro_do_recorte",
        F.coalesce(F.col("data_coleta").between(F.lit(DATA_INICIAL), F.lit(DATA_FINAL)), F.lit(False)),
    )
)
...
conciliacao_municipio = (
    preco_silver.filter(F.col("dentro_do_recorte")).groupBy(
        "municipio_origem", "municipio_norm", "uf", "codigo_ibge", "municipio_conciliado"
    )
    .agg(
...
logistica_silver = (
    logistica_silver.withColumn("periodo_valido", F.col("data_referencia").isNotNull())
    .withColumn(
        "dentro_do_recorte",
        F.coalesce(F.col("data_referencia").between(F.lit(DATA_INICIAL), F.lit(DATA_FINAL)), F.lit(False)),
    )
    .withColumn("uf_destino_valida", F.col("uf").rlike(r"^[A-Z]{2}$"))
...
simp_raw = normalize_headers(bronze_lote("bronze_venda_simp_liquidos"))
simp_base = simp_raw.select(
    "lote_id",
    "arquivo_origem",
    "data_ingestao",
    "fonte_carga",
    F.trim(pick(simp_raw, ["ano"], "ano").cast("string")).cast("int").alias("ano"),
    F.trim(pick(simp_raw, ["mes"], "mes").cast("string")).cast("int").alias("mes"),
    F.trim(pick(simp_raw, ["agente_regulado"], "agente_regulado")).alias("agente_regulado"),
    F.trim(pick(simp_raw, ["codigo_do_produto"], "codigo_do_produto")).alias("codigo_produto"),
    F.trim(pick(simp_raw, ["nome_do_produto"], "nome_do_produto")).alias("nome_produto"),
    F.trim(pick(simp_raw, ["descricao_do_produto"], "descricao_do_produto")).alias("produto_origem"),
    normalized_text(pick(simp_raw, ["regiao_origem"], "regiao_origem", required=False)).alias("regiao_origem"),
    normalized_text(pick(simp_raw, ["uf_origem"], "uf_origem", required=False)).alias("uf_origem"),
    normalized_text(
        pick(simp_raw, ["regiao_destinatario"], "regiao_destinatario", required=False)
    ).alias("regiao_destinatario"),
    normalized_text(pick(simp_raw, ["uf_destino"], "uf_destino")).alias("uf"),
    F.trim(pick(simp_raw, ["mercado_destinatario"], "mercado_destinatario")).alias("mercado_destinatario"),
    decimal_from_text(
        pick(simp_raw, ["quantidade_de_produto_mil_m3"], "quantidade_de_produto_mil_m3")
    ).alias("volume_mil_m3"),
)
simp_base = (
    simp_base.withColumn("data_referencia", F.make_date(F.col("ano"), F.col("mes"), F.lit(1)))
    .withColumn("produto_origem_normalizado", normalized_text(F.col("produto_origem")))
    .withColumn("agente_chave", normalized_key(F.col("agente_regulado")))
    .withColumn("volume_simp_litros", F.col("volume_mil_m3") * F.lit(1_000_000))
)
simp_silver = apply_product_map(simp_base, "SIMP_LIQUIDOS")
simp_silver = (
    simp_silver.withColumn("periodo_valido", F.col("data_referencia").isNotNull())
    .withColumn(
        "dentro_do_recorte",
        F.coalesce(F.col("data_referencia").between(F.lit(DATA_INICIAL), F.lit(DATA_FINAL)), F.lit(False)),
    )
    .withColumn("uf_destino_valida", F.col("uf").rlike(r"^[A-Z]{2}$"))
    .withColumn("agente_preenchido", F.col("agente_chave") != "")
    .withColumn("mercado_destinatario_preenchido", F.trim(F.col("mercado_destinatario")) != "")
    .withColumn("volume_valido", F.col("volume_simp_litros").isNotNull() & (F.col("volume_simp_litros") >= 0))
    .withColumn(
        "chave_venda_simp",
        F.sha2(
            F.concat_ws(
...
simp_silver = add_duplicate_flag(simp_silver, "chave_venda_simp")
write_delta("silver_venda_simp_liquidos", simp_silver)
...
dim_empresa_vendedora = (
    logistica_silver.filter(F.col("dentro_do_recorte")).groupBy("vendedor_chave")
    .agg(
...
dim_agente_simp = (
    simp_silver.filter(F.col("dentro_do_recorte")).groupBy("agente_chave")
    .agg(
        F.first("agente_regulado", ignorenulls=True).alias("agente_regulado"),
        F.min("data_referencia").alias("primeira_referencia"),
        F.max("data_referencia").alias("ultima_referencia"),
    )
)
dim_mercado_destino = (
    simp_silver.filter(F.col("dentro_do_recorte"))
    .select(F.coalesce(F.col("mercado_destinatario"), F.lit("NAO INFORMADO")).alias("mercado_destinatario"))
    .distinct()
)
ufs_venda = dim_municipio.select("uf", "grande_regiao")
ufs_preco = (
    preco_silver.filter(F.col("dentro_do_recorte"))
    .select("uf")
    .withColumn("grande_regiao", F.lit(None).cast("string"))
)
ufs_logistica = (
    logistica_silver.filter(F.col("dentro_do_recorte"))
    .select("uf")
    .withColumn("grande_regiao", F.lit(None).cast("string"))
)
ufs_simp = (
...
dim_uf = (
    ufs_venda.unionByName(ufs_preco)
    .unionByName(ufs_logistica)
    .unionByName(ufs_simp)
    .filter(F.col("uf").rlike(r"^[A-Z]{2}$"))
    .groupBy("uf")
...
dim_bandeira = (
    ...
    .unionByName(
        preco_silver.filter(F.col("dentro_do_recorte")).select(
            F.coalesce(F.col("bandeira"), F.lit("NAO INFORMADA")).alias("bandeira")
        )
    )
    .distinct()
...
dim_tempo = spark.sql(
    ...
    f"DATE '{DATA_INICIAL.isoformat()}', DATE '{DATA_FINAL.isoformat()}', INTERVAL 1 DAY)) AS data"
)
dim_tempo = (
...
write_delta("gold_dim_agente_simp", dim_agente_simp)
write_delta("gold_dim_mercado_destino", dim_mercado_destino)
...
write_delta("gold_fato_preco_coletado", preco_silver.filter(F.col("dentro_do_recorte")))
write_delta("gold_fato_venda_municipio_anual", venda_silver.filter(F.col("dentro_do_recorte")))
...
simp_venda = simp_silver.filter(
    F.col("dentro_do_recorte")
    & F.col("uf_destino_valida")
    & F.col("agente_preenchido")
    & F.col("volume_valido")
    & (F.col("produto_analitico") != "NAO_MAPEADO")
    & F.col("usar_analise_principal")
)
simp_agente = (
    simp_venda.groupBy(*chaves_mercado, "agente_chave")
    .agg(
        F.first("agente_regulado", ignorenulls=True).alias("agente_regulado"),
        F.sum("volume_simp_litros").alias("volume_simp_litros"),
        F.countDistinct("mercado_destinatario").alias("mercados_destino_atendidos"),
        F.countDistinct("uf_origem").alias("ufs_origem"),
        F.max("lote_id").alias("lote_id"),
    )
    .withColumn("volume_simp_m3", F.col("volume_simp_litros") / F.lit(1000))
)
resumo_simp_uf = simp_agente.groupBy(*chaves_mercado).agg(
    F.sum("volume_simp_litros").alias("volume_simp_total_uf_litros"),
    F.sum(F.when(F.col("volume_simp_litros") > 0, F.col("volume_simp_litros")).otherwise(F.lit(0))).alias(
        "volume_simp_positivo_uf_litros"
    ),
    F.countDistinct(F.when(F.col("volume_simp_litros") > 0, F.col("agente_chave"))).alias(
        "agentes_simp_positivos"
    ),
    F.max("lote_id").alias("lote_id"),
)
simp_participacao = (
    simp_agente.join(resumo_simp_uf.drop("lote_id"), chaves_mercado, "inner")
    .filter((F.col("volume_simp_litros") > 0) & (F.col("volume_simp_positivo_uf_litros") > 0))
    .withColumn(
        "participacao_simp_pct",
        F.col("volume_simp_litros") / F.col("volume_simp_positivo_uf_litros") * F.lit(100),
    )
)
simp_ranking_window = Window.partitionBy(*chaves_mercado).orderBy(
    F.col("volume_simp_litros").desc(), F.col("agente_chave").asc()
)
simp_participacao = simp_participacao.withColumn("posicao_simp", F.row_number().over(simp_ranking_window))
write_delta("gold_fato_venda_simp_agente_uf_mes", simp_participacao)

simp_mercado_destino = (
    simp_venda.groupBy(*chaves_mercado, "mercado_destinatario")
    .agg(
        F.sum("volume_simp_litros").alias("volume_simp_litros"),
        F.countDistinct("agente_chave").alias("agentes_simp_distintos"),
        F.max("lote_id").alias("lote_id"),
    )
    .join(resumo_simp_uf.drop("lote_id"), chaves_mercado, "left")
    .withColumn(
        "participacao_mercado_destino_pct",
        F.when(
            F.col("volume_simp_positivo_uf_litros") > 0,
            F.col("volume_simp_litros") / F.col("volume_simp_positivo_uf_litros") * F.lit(100),
        ),
    )
)
write_delta("gold_fato_venda_simp_mercado_uf_mes", simp_mercado_destino)

simp_concentracao = simp_participacao.groupBy(*chaves_mercado).agg(
    F.countDistinct("agente_chave").alias("agentes_simp_positivos"),
    F.sum(F.pow(F.col("participacao_simp_pct") / F.lit(100), F.lit(2)) * F.lit(10000)).alias("hhi_simp"),
    F.sum(F.when(F.col("posicao_simp") <= 3, F.col("participacao_simp_pct")).otherwise(F.lit(0))).alias(
        "participacao_simp_top3_pct"
    ),
    F.max(F.when(F.col("posicao_simp") == 1, F.col("agente_regulado"))).alias("agente_lider_simp"),
    F.max(F.when(F.col("posicao_simp") == 1, F.col("participacao_simp_pct"))).alias(
        "participacao_simp_lider_pct"
    ),
    F.max("lote_id").alias("lote_id"),
)
simp_mart = resumo_simp_uf.alias("s").join(preco_uf_mes.alias("p"), chaves_mercado, "full_outer")
simp_mart = simp_mart.select(
    *[F.coalesce(F.col(f"s.{key}"), F.col(f"p.{key}")).alias(key) for key in chaves_mercado],
    F.col("s.volume_simp_total_uf_litros").alias("volume_simp_total_uf_litros"),
    F.col("s.volume_simp_positivo_uf_litros").alias("volume_simp_positivo_uf_litros"),
    F.col("s.agentes_simp_positivos").alias("agentes_simp_positivos"),
    F.col("p.mediana_preco").alias("mediana_preco"),
    F.col("p.media_preco").alias("media_preco"),
    F.col("p.p10_preco").alias("p10_preco"),
    F.col("p.p90_preco").alias("p90_preco"),
    ...
    F.col("p.qtd_postos").alias("qtd_postos"),
    F.col("p.qtd_municipios").alias("qtd_municipios"),
    F.coalesce(F.col("s.lote_id"), F.col("p.lote_id")).alias("lote_id"),
).join(simp_concentracao.drop("lote_id", "agentes_simp_positivos"), chaves_mercado, "left")
simp_mart = simp_mart.withColumn("preco_disponivel", F.col("mediana_preco").isNotNull()).withColumn(
    "volume_simp_disponivel", F.col("volume_simp_total_uf_litros").isNotNull()
)
write_delta("gold_mart_simp_uf_mes", simp_mart)

comparacao_simp_logistica = resumo_simp_uf.alias("s").join(
    resumo_venda_uf.alias("l"), chaves_mercado, "full_outer"
).select(
    *[F.coalesce(F.col(f"s.{key}"), F.col(f"l.{key}")).alias(key) for key in chaves_mercado],
    F.col("s.volume_simp_total_uf_litros").alias("volume_simp_litros"),
    F.col("l.volume_total_liquido_uf_litros").alias("volume_logistica_litros"),
    F.col("s.agentes_simp_positivos").alias("agentes_simp_positivos"),
    F.col("l.vendedores_positivos").alias("vendedores_logistica_positivos"),
    F.coalesce(F.col("s.lote_id"), F.col("l.lote_id")).alias("lote_id"),
).withColumn(
    "diferenca_litros", F.col("volume_simp_litros") - F.col("volume_logistica_litros")
).withColumn(
    "diferenca_pct_logistica",
    F.when(
        F.col("volume_logistica_litros") != 0,
        F.col("diferenca_litros") / F.col("volume_logistica_litros") * F.lit(100),
    ),
).withColumn(
    "status_comparacao",
    F.when(
        F.col("volume_simp_litros").isNull() | F.col("volume_logistica_litros").isNull(),
        F.lit("SEM_BASE_COMPARAVEL"),
    )
    .when(F.abs(F.col("diferenca_pct_logistica")) <= 1, F.lit("PROXIMO_EM_VOLUME"))
    .otherwise(F.lit("DIVERGENCIA_DE_ESCOPO")),
)
write_delta("gold_comparacao_simp_logistica_uf_mes", comparacao_simp_logistica)

comparacao_agente_simp_logistica = simp_participacao.alias("s").join(
    participacao.alias("l"),
    (F.col("s.data_referencia") == F.col("l.data_referencia"))
    & (F.col("s.ano") == F.col("l.ano"))
    & (F.col("s.mes") == F.col("l.mes"))
    & (F.col("s.uf") == F.col("l.uf"))
    & (F.col("s.produto_analitico") == F.col("l.produto_analitico"))
    & (F.col("s.agente_chave") == F.col("l.vendedor_chave")),
    "full_outer",
).select(
    *[
        F.coalesce(F.col(f"s.{key}"), F.col(f"l.{key}")).alias(key)
        for key in chaves_mercado
    ],
    F.coalesce(F.col("s.agente_chave"), F.col("l.vendedor_chave")).alias("agente_chave"),
    F.col("s.agente_regulado").alias("agente_regulado_simp"),
    F.col("l.empresa_canonica").alias("empresa_canonica_logistica"),
    F.col("s.volume_simp_litros").alias("volume_simp_litros"),
    F.col("l.volume_liquido_litros").alias("volume_logistica_litros"),
    F.col("s.participacao_simp_pct").alias("participacao_simp_pct"),
    F.col("l.participacao_pct").alias("participacao_logistica_pct"),
    F.col("s.posicao_simp").alias("posicao_simp"),
    F.col("l.posicao").alias("posicao_logistica"),
    F.coalesce(F.col("s.lote_id"), F.col("l.lote_id")).alias("lote_id"),
).withColumn(
    "diferenca_participacao_pp",
    F.col("participacao_simp_pct") - F.col("participacao_logistica_pct"),
).withColumn(
    "status_correspondencia",
    F.when(F.col("volume_simp_litros").isNull(), F.lit("SOMENTE_LOGISTICA"))
    .when(F.col("volume_logistica_litros").isNull(), F.lit("SOMENTE_SIMP"))
    .otherwise(F.lit("CORRESPONDENCIA_NOME_NORMALIZADO")),
)
write_delta("gold_comparacao_simp_logistica_agente_uf_mes", comparacao_agente_simp_logistica)


# COMMAND ----------

produtos_principais = dim_produto.filter(F.col("usar_analise_principal")).select("produto_analitico").distinct()
venda_natural = venda_silver.filter(F.col("dentro_do_recorte") & F.col("volume_valido")).withColumn(
...
reconciliacao_etapas = spark.createDataFrame(
    [
        ...
        (
            "bronze_simp_liquidos",
            bronze_lote("bronze_venda_simp_liquidos").count(),
            "Registros da base SIMP usada no painel de combustíveis líquidos.",
        ),
        ...
        ("silver_venda_simp", simp_silver.count(), "Vendas do SIMP tipadas no período selecionado."),
        ("gold_mercado_uf_mes", mercado_uf.count(), "Combinações UF-mês-produto disponíveis no mart estadual."),
        ("gold_mart_simp_uf_mes", simp_mart.count(), "Combinações UF-mês-produto disponíveis no mart do SIMP."),
        (
            "gold_comparacao_simp_logistica",
            comparacao_simp_logistica.count(),
            "Comparações mensais entre SIMP e Logística 02 por UF e produto.",
        ),
        (
    ...
    "execucao_id", F.lit(EXECUCAO_ID)
)
write_delta("gold_reconciliacao_etapas", reconciliacao_etapas)

controle_fim = spark.createDataFrame(
    [
        (
            EXECUCAO_ID,
            LOTE_ID,
...
append_delta("gold_controle_execucao", controle_fim)
