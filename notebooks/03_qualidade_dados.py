...
dbutils.widgets.text(
    "config_root",
    "/Volumes/workspace/anp_combustiveis_br_2022_2026/anp/config",
...
controle_execucao = spark.table(table("gold_controle_execucao"))
latest_control = (
    controle_execucao
    .orderBy(
        F.col("data_evento").desc(),
        F.when(F.col("status") == "CONCLUIDA", F.lit(1)).otherwise(F.lit(0)).desc(),
        F.col("execucao_id").desc(),
    )
    .select("lote_id", "execucao_id", "status")
    .first()
)
if latest_control is None or latest_control["status"] != "CONCLUIDA":
    raise ValueError("A última transformação não foi concluída. Execute o notebook 02 novamente.")

LOTE_ID = latest_control["lote_id"]
EXECUCAO_ID = latest_control["execucao_id"]
etapas_execucao = spark.table(table("gold_reconciliacao_etapas"))
if "execucao_id" not in etapas_execucao.columns or (
    etapas_execucao.filter(F.col("execucao_id") == EXECUCAO_ID).limit(1).count() == 0
):
    raise ValueError("As etapas da última transformação não estão disponíveis. Execute o notebook 02 novamente.")
...
PROFILE_TABLES = [
    ...
    "gold_dim_agente_simp",
    "gold_dim_mercado_destino",
    "gold_dim_revenda",
    "gold_dim_bandeira",
    "gold_dim_lote_carga",
    ...
    "gold_fato_venda_simp_agente_uf_mes",
    "gold_fato_venda_simp_mercado_uf_mes",
    "gold_mart_simp_uf_mes",
    "gold_comparacao_simp_logistica_uf_mes",
    "gold_comparacao_simp_logistica_agente_uf_mes",
...
qualidade = [
    ...
    count_rule(
        "silver_preco_coletado",
        "PRECO_007",
        "Data de coleta ausente ou inválida.",
        ~F.col("periodo_valido"),
    ),
    count_rule(
        "silver_preco_coletado",
        "PRECO_008",
        "Registro fora do recorte analítico preservado na camada Silver.",
    ...
    count_rule(
        "silver_venda_municipio",
        "VENDA_MUNICIPIO_005",
        "Ano de referência ausente ou inválido.",
        ~F.col("periodo_valido"),
    ),
    count_rule(
        "silver_venda_municipio",
        "VENDA_MUNICIPIO_006",
        "Registro fora do recorte municipal preservado na camada Silver.",
        ~F.col("dentro_do_recorte"),
        "INFORMATIVA",
    ),
    count_rule(
    ...
    count_rule(
        "silver_venda_simp_liquidos",
        "SIMP_003",
        "Agente regulado ausente após normalização.",
        ~F.col("agente_preenchido"),
    ),
    count_rule(
        "silver_venda_simp_liquidos",
        "SIMP_004",
        "Mercado destinatário ausente.",
        ~F.col("mercado_destinatario_preenchido"),
    ),
    count_rule(
        "silver_venda_simp_liquidos",
        "SIMP_005",
        "Volume do SIMP ausente ou negativo.",
        ~F.col("volume_valido"),
    ),
    count_rule(
        "silver_venda_simp_liquidos",
        "SIMP_006",
        "Produto principal do SIMP sem regra de mapeamento.",
        F.col("produto_origem_normalizado").isin("GASOLINA C COMUM", "ETANOL HIDRATADO COMUM")
        & (F.col("produto_analitico") == "NAO_MAPEADO"),
    ),
    count_rule(
        "silver_venda_simp_liquidos",
        "SIMP_007",
        "Repetição da chave mês, origem, destino, produto, agente e mercado.",
        F.col("duplicidade_chave"),
        "INFORMATIVA",
    ),
    count_rule(
    ...
    count_rule(
        "gold_comparacao_simp_logistica_uf_mes",
        "SIMPCOMP_001",
        "Diferença de escopo entre o SIMP e a Logística 02 na combinação mensal.",
        F.col("status_comparacao") == "DIVERGENCIA_DE_ESCOPO",
        "INFORMATIVA",
    ),
    count_rule(
        "gold_comparacao_simp_logistica_uf_mes",
        "SIMPCOMP_002",
        "Combinação disponível em apenas uma das fontes comparadas.",
        F.col("status_comparacao") == "SEM_BASE_COMPARAVEL",
        "INFORMATIVA",
    ),
    count_rule(
        "gold_comparacao_simp_logistica_agente_uf_mes",
        "SIMPCOMP_003",
        "Agente presente em uma fonte sem correspondência pelo nome normalizado na outra.",
        F.col("status_correspondencia") != "CORRESPONDENCIA_NOME_NORMALIZADO",
        "INFORMATIVA",
    ),
]
...
participacao_simp_soma = (
    spark.table(table("gold_fato_venda_simp_agente_uf_mes"))
    .groupBy("data_referencia", "uf", "produto_analitico")
    .agg(F.sum("participacao_simp_pct").alias("soma_participacao_simp_pct"))
)
contagem_participacao_simp = participacao_simp_soma.agg(
    F.count(F.lit(1)).alias("total"),
    F.sum(
        F.when(F.abs(F.col("soma_participacao_simp_pct") - 100) > 0.01, F.lit(1)).otherwise(F.lit(0))
    ).alias("afetados"),
).first()
qualidade.append(
    result(
        "SIMP_PARTICIPACAO_001",
        "gold_fato_venda_simp_agente_uf_mes",
        "Soma das participações do SIMP diferente de 100% dentro da tolerância de arredondamento.",
        int(contagem_participacao_simp["afetados"] or 0),
        int(contagem_participacao_simp["total"] or 0),
    )
)
...
).withColumn("lote_id", F.lit(LOTE_ID)).withColumn("execucao_id", F.lit(EXECUCAO_ID))
